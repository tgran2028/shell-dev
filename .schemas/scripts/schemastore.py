...
def get_schema(name: str, raw: bool = False) -> Schema:
    """
...
def search_schema(
    ...
    if fuzzy:
        schema = fuzzy_search(name)
    else:
        schema = get_schema(name)

    if fmt == OutputFormat.JSON:
        data = json.dumps(schema.to_dict(), indent=indent)
        s = Syntax(data, lexer="json", line_numbers=False)
        console.print(s)
    elif fmt == OutputFormat.YAML:
        data = yaml.dump(
            schema.to_dict(), default_flow_style=False, sort_keys=False, indent=indent
        )
        s = Syntax(data, lexer="yaml", line_numbers=False)
        console.print(s)
    elif fmt == OutputFormat.LIST:
