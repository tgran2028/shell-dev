...
print_bat_diff() {
  ...
  if "$OPT_STAGED"; then
    if false && "$SUPPORTS_DELTA"; then
      ...
      if [[ ${#difftext} -gt 0 ]]; then
        "batcat" --language=diff --file-name="${files[0]}" - "${BAT_ARGS[@]}" <<< "$difftext"
      fi
    fi
    return $?
  fi
  ...
  if "$SUPPORTS_BAT_DIFF"; then
    "git" diff "${GIT_ARGS[@]}" --name-only -z "${files[0]}" \
      | xargs --null "batcat" --diff --diff-context="$OPT_CONTEXT" "${BAT_ARGS[@]}"
  else
    "git" diff "${GIT_ARGS[@]}" "${files[0]}" | "batcat" --language=diff - "${BAT_ARGS[@]}"
  fi
}
